...
namespace Whitespace.net {
			...
			for (int i = 0; i < program.Instructions.Count; i++) {
				...
				switch (instr.op) {
					...
					case Instruction.OpCode.wrc:
						...
						ilg.EmitCall(OpCodes.Call, Type.GetType("System.Console").GetMethod("Write", new Type[]{ typeof(char) }), null);
						stackHeight--;
						break;
					case Instruction.OpCode.wri:
						...
						ilg.Emit(OpCodes.Unbox, typeof(int));
						ilg.Emit(OpCodes.Ldind_I4);
						ilg.EmitCall(OpCodes.Call, Type.GetType("System.Console").GetMethod("Write", new Type[]{ typeof(int) }), null);
						stackHeight--;
						break;
					case Instruction.OpCode.rdc:
						...
						ilg.Emit(OpCodes.Ldloc, evstack);
						ilg.EmitCall(OpCodes.Callvirt, PopM, null);
						ilg.EmitCall(OpCodes.Call, Type.GetType("System.Console").GetMethod("Read", new Type[]{}), null);
						ilg.Emit(OpCodes.Box, typeof(int));
						ilg.EmitCall(OpCodes.Callvirt, typeof(Hashtable).GetMethod("set_Item", new Type[]{ typeof(object), typeof(object) }), null);
						stackHeight--;
						break;
	...
	class MainCompiler {
		static void Main(string[] args) {
			...
			ModuleBuilder mb = ab.DefineDynamicModule(args[1]+".exe");
			TypeBuilder tb = mb.DefineType("WSMain", TypeAttributes.Class | TypeAttributes.Public);
			MethodBuilder mtb = tb.DefineMethod("Main", MethodAttributes.Public | MethodAttributes.Static, typeof(void), new Type[] { typeof(string[]) });
			ILGenerator ilg = mtb.GetILGenerator();

			WSProgram prg = new WSProgram();
			p.Parse(prg);
			WSCompiler comp = new WSCompiler(prg);
			comp.Compile(ilg);
